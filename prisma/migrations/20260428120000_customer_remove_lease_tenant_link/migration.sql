-- RedefineTables: remove vínculo cliente ↔ locação/locatário; enum sem LEASE_CONTRACT/TENANT
...
INSERT INTO "new_Customer" (
    "id", "branchId", "operationLinkKind", "propertyId", "parkingFacilityId",
...
SELECT
    "id",
    CASE WHEN "operationLinkKind" IN ('LEASE_CONTRACT', 'TENANT') THEN NULL ELSE "branchId" END,
    CASE WHEN "operationLinkKind" IN ('LEASE_CONTRACT', 'TENANT') THEN 'NONE' ELSE "operationLinkKind" END,
    CASE WHEN "operationLinkKind" IN ('LEASE_CONTRACT', 'TENANT') THEN NULL ELSE "propertyId" END,
    CASE WHEN "operationLinkKind" IN ('LEASE_CONTRACT', 'TENANT') THEN NULL ELSE "parkingFacilityId" END,
    "kind", "name", "tradeName", "documentType", "document", "phone", "email",
    "street", "number", "complement", "neighborhood", "city", "state", "zip",
    "billingProfile", "preferredDueDay", "suggestedAmountCents", "preferredPaymentMethodsJson", "notes", "active", "createdAt", "updatedAt"
FROM "Customer";
