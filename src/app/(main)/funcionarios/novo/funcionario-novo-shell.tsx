...
import { TeacherForm, type TeacherFormInitial } from "./teacher-form";
import { TeacherSearchPanel, type TeacherPickerRow } from "./teacher-search-panel";

type Props = {
  effectiveFuncionarioId: string | null;
  ...
  ok: boolean;
  lastSavedTeacherId: string | undefined;
  initialForm: TeacherFormInitial | undefined;
  jobRoleOptions: readonly { id: string; name: string }[];
  employeeCategoryOptions: readonly { id: string; name: string }[];
};

export function FuncionarioNovoShell({
  ...
  const showHeader =
    revealFichaFromParams || (!effectiveFuncionarioId && cadastroOpen);
  const fichaBelowList =
    Boolean(effectiveFuncionarioId) && revealFichaFromParams && showHeader;
  const fichaBelowSearchOnly = showHeader && !effectiveFuncionarioId;
  const fichaPanelOpen = Boolean(effectiveFuncionarioId && revealFichaFromParams);
  ...
  useEffect(() => {
    function onOpenCadastroEdicao() {
      ...
      window.setTimeout(() => {
        document
          .getElementById("cadastro-edicao")
  ...
  return (
    <>
      <div className="mt-0 pt-1 sm:pt-2">
        <TeacherSearchPanel
          teachers={teachers}
          currentTeacherId={effectiveFuncionarioId}
          ...
          <div className="flex gap-3">
            ...
            <div>
              ...
              <p className="mt-0.5 text-sm text-success-fg/85">
                Os dados foram registrados. Você pode cadastrar outro(a) funcionário(a) ou continuar
                editando.
              </p>
            </div>
          </div>
      ...
      <div id="cadastro-edicao" className="scroll-mt-24">
        <CadastroEdicaoCollapsible
          ...
          showLabel="Cadastrar ou editar funcionário(a)"
          heading={
            <div>
              <h2 className="text-lg font-semibold text-ink">Cadastro e edição</h2>
              <p className="mt-1 text-sm text-muted">
                Identificação, função, contato, endereço, CTPS e dados de vínculo trabalhista.
              </p>
            </div>
          }
        >
          <TeacherForm
            key={effectiveFuncionarioId ?? "novo"}
            lastSavedTeacherId={lastSavedTeacherId}
            initial={initialForm}
