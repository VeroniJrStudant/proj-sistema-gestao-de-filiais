...
export default async function FiliaisPage({
  ...
  const selectedId = empty(typeof sp.filial === "string" ? sp.filial : "");
  ...
  const selected = selectedId
    ? await prisma.branch.findUnique({ where: { id: selectedId } })
    : null;
      ...
      <div className="mx-auto w-full min-w-0 max-w-6xl space-y-6 sm:space-y-8">
        <header className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <h1 className="text-lg font-semibold text-ink sm:text-xl">Filiais</h1>
            <p className="mt-1 text-sm text-muted">
              Cadastro das unidades. Use para vincular imóveis, estacionamentos e relatórios financeiros por filial.
            </p>
          </div>
          <Link
            href="/filiais"
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-12">
          <section className="min-w-0 rounded-2xl border border-line-soft bg-elevated-2 p-4 shadow-sm lg:col-span-5">
            ...
            {branches.length === 0 ? (
              <p className="mt-3 text-sm text-muted">Ainda não há filiais cadastradas.</p>
            ) : (
              <ul className="mt-3 space-y-2">
                {branches.map((b) => {
                  const href = `/filiais?filial=${encodeURIComponent(b.id)}`;
                    ...
                    <li key={b.id}>
                      ...
                      >
                        ...
                        <div className="mt-0.5 text-xs text-muted">
                          {b.code ? (
                            <>
                              Código <span className="tabular-nums text-subtle">{b.code}</span>
                            </>
                          ) : (
                            <span className="text-subtle">Sem código</span>
                          )}
                          {b.city?.trim() ? (
                            <>
                              {" · "}
          ...
          <section className="min-w-0 rounded-2xl border border-line-soft bg-elevated-2 p-4 shadow-sm lg:col-span-7">
            ...
            <form
              action={async (fd) => {
                "use server";
                const r = await saveBranch(fd);
                if (!r.ok) return;
                redirect(`/filiais?filial=${encodeURIComponent(r.id)}&ok=1`);
              }}
            ...
            >
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <label className="block sm:col-span-2">
                  <span className="block text-xs font-medium text-muted">Documento (opcional)</span>
                  <input
                    name="document"
