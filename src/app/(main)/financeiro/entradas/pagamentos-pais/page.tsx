...
import { DirectCheckoutPanel } from "../../direta/direct-checkout-panel";

export default async function PagamentosPaisPage() {
  const integration = getDirectIntegrationStatus();
  const enrollmentYear = new Date().getFullYear();

  const [studentsRaw, shiftsRaw, classRoomsRaw, teachersRaw, suppliersRaw] = await Promise.all([
    ...
    <ModuleGate id="finance">
      <div className="mx-auto min-w-0 max-w-4xl space-y-6 px-3 sm:px-4">
        <nav className="text-sm text-muted">
          ...
          <Link href="/financeiro/entradas" className="text-accent-muted underline decoration-dotted hover:text-accent">
            Entradas
          </Link>
          <span className="text-subtle"> · </span>
          <span className="text-ink">Pagamentos para famílias</span>
        </nav>

        <header>
          <h1 className="text-xl font-semibold text-ink">Pagamentos para pais e responsáveis</h1>
          <p className="mt-1 text-sm text-muted">
            Gere cobranças para mensalidades e demais valores devidos à creche:{" "}
            <span className="text-ink">PIX com liquidação imediata</span>,{" "}
            <span className="text-ink">documento de débito</span> (impressão e e-mail),{" "}
            <span className="text-ink">boleto bancário</span>,{" "}
            <span className="text-ink">cartão à vista, parcelado ou recorrente</span> e pedido de{" "}
            <span className="text-ink">nota fiscal</span>. Vincule ao aluno quando fizer sentido para o
            controle no painel. Sem provedor (PSP) configurado, as solicitações ficam como lançamentos
            pendentes até a integração ser concluída.
          </p>
        </header>

        <p className="rounded-xl border border-line-soft bg-elevated px-4 py-3 text-sm text-muted">
          Quando o pagamento já tiver sido recebido na secretaria (PIX avulso, dinheiro, TED etc.), registre em{" "}
          <Link
