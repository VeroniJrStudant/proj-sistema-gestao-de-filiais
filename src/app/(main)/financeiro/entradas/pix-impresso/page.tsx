import { PixPrintPanel } from "./pix-print-panel";
import { ModuleGate } from "@/components/module-gate";
import { getSchoolDisplayName } from "@/lib/company-settings";
import { hasSchoolPixKey } from "@/lib/pix/school-pix-config";
...
export default async function PixImpressoPage() {
  const schoolLabel = await getSchoolDisplayName();
  const hasPixKey = await hasSchoolPixKey();
    ...
    <ModuleGate id="finance">
      <div className="mx-auto min-w-0 max-w-4xl space-y-6 px-3 sm:px-4">
        <nav className="text-sm text-muted">
          ...
          <span className="text-subtle"> · </span>
          <span className="text-ink">PIX para impressão (QR + código)</span>
        </nav>

        <header>
          <h1 className="text-xl font-semibold text-ink">PIX para impressão</h1>
          <p className="mt-1 text-sm text-muted">
            Gere o <span className="text-ink">QR Code</span>, o <span className="text-ink">Pix copia e cola</span> e um{" "}
            <span className="text-ink">código de barras</span> de referência para a família pagar de qualquer lugar.
            O valor é incluído no BR Code (padrão Banco Central). A chave PIX da unidade é definida pelos
            administradores em Dados da empresa (menu Financeiro) ou pelas variáveis do servidor
            (SCHOOL_PIX_KEY e afins).
          </p>
        </header>
